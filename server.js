...
wss.on('connection', socket => {
  ...
  socket.on('message', raw => {
    try {
      ...
      if (message.type === 'SELECT_MARKET' && typeof message.symbol === 'string') {
      ...
      if (message.type === 'PLACE_TRADE' && message.input) {
        ...
        broadcastState()
        socket.send(JSON.stringify({ type: 'ACK', ok: true, action: state.lastAction, tradeId: result.trade.id }))
        return
      }
...
function createInitialState() {
  return {
    markets: Object.fromEntries(MARKET_SEEDS.map(seed => [seed.symbol, createMarket(seed)])),
    balance: STARTING_BALANCE,
    realizedPnl: 1471.35,
    openTrades: [],
    closedTrades: [
      sampleTrade('TRD-8821', 'VOL-75', 'Volatility 75 Index', 'rise-fall', 'rise', 50, 8120.44, 8243.12, 8, 30, 1.82, 'won', 41, 0),
      sampleTrade('TRD-8820', 'BOOM-500', 'Boom 500 Index', 'rise-fall', 'fall', 40, 13201, 12891.44, 5, 30, 1.82, 'won', 32.8, 1),
      sampleTrade('TRD-8819', 'CRASH-300', 'Crash 300 Index', 'even-odd', 'odd', 35, 5890.2, 5674.8, 2, 20, 1.8, 'lost', -35, 2),
    ],
...
function createMarket(seed) {
  ...
  for (let index = 1; index < 90; index++) {
    ...
    history.push(nextPrice(last, seed.volatility, seed.drift, seed.basePrice, index))
  }
...
function placeTrade(currentState, input) {
  ...
  const expirySeconds = Math.max(10, Math.round(Number(input.expirySeconds || 30)))
  const trade = {
...
function forceCloseTrade(currentState, tradeId) {
  ...
  currentState.realizedPnl = round2(currentState.realizedPnl + (settled.profit || 0))
...
function sampleTrade(id, symbol, marketName, contractType, direction, stake, entryPrice, exitPrice, entryDigit, expirySeconds, payoutMultiplier, result, profit, minuteOffset) {
  return {
    id,
    symbol,
    marketName,
