...
function CandleChart({
  ...
  const toY = (price: number) =>
    PADDING.top + chartH - ((price - allLow) / range) * chartH
  ...
  return (
    ...
    >
      ...
      <circle
        ...
        fill={isUp ? "#22C55E" : "#EF4444"}
        style={{
          filter: `drop-shadow(0 0 5px ${isUp ? "#22C55E" : "#EF4444"})`,
        }}
      />
    </svg>
  )
...
function LineChart({
  ...
  const toX = (index: number) =>
    PADDING.left + (index / Math.max(1, series.length - 1)) * chartW
  ...
  return (
    ...
    >
      ...
      <circle
        ...
        cy={toY(series[series.length - 1] ?? livePrice)}
        r="4"
        fill={isUp ? "#22C55E" : "#EF4444"}
        style={{
          filter: `drop-shadow(0 0 5px ${isUp ? "#22C55E" : "#EF4444"})`,
...
export default function Terminal() {
  ...
  const [contractType, setContractType] =
    useState<"rise-fall" | "even-odd" | "match-differ">("rise-fall")

  const [prediction, setPrediction] =
    useState<"rise" | "fall" | "even" | "odd" | "match" | "differ">("rise")

  const [stake, setStake] = useState("50")

  const [expirySeconds, setExpirySeconds] = useState("30")

  const [forcedOutcome, setForcedOutcome] = useState<"none" | "won" | "lost">(
    adminPolicy.forceNextOutcome ?? "none",
  )
  ...
  const feedback =
    lastError ??
    lastAction ??
    "Binary contracts settle automatically at expiry."
    ...
    return (
      ...
      >
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 13, color: "#71717A", marginTop: 6 }}>
              Simulation controls for AdminSim accounts only. Trading controls
        ...
        <div
          style={{
            display: "grid",
            gridTemplateColumns: "minmax(360px, 520px)",
            gap: 16,
          }}
        >
          <div
            ...
            <div style={{ display: "grid", gap: 10 }}>
              <div>
                ...
                <select
                  ...
                  value={forcedOutcome}
                  onChange={(e) =>
                    setForcedOutcome(e.target.value as "none" | "won" | "lost")
                  }
                >
              ...
              <button
                ...
                disabled={!capabilities.canViewAdminPanel}
              >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 20, marginLeft: 8 }}>
          {[
            {
              label: "O",
              val: candles[0]?.open.toFixed(2) ?? livePrice.toFixed(2),
            },

            {
              label: "H",
              val: Math.max(...candles.slice(-20).map((c) => c.high)).toFixed(
                2,
              ),
            },
            ...
            {
              label: "L",
              val: Math.min(...candles.slice(-20).map((c) => c.low)).toFixed(2),
            },
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
          ...
          {marketList.slice(0, 8).map((market) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className="font-mono-data"
                  style={{ fontSize: 11, color: "#FFFFFF", marginBottom: 1 }}
                >
                ...
                <div
                  ...
                  style={{
                    fontSize: 10,
                    color: live.changePct >= 0 ? "#22C55E" : "#EF4444",
                  }}
                >
                  {live.changePct >= 0 ? "+" : ""}
        ...
        >
          ...
          >
            {chartMode === "candles" ? (
              ...
              <LineChart history={liveMarket.history} livePrice={livePrice} />
            )}
          </div>
          {chartMode === "candles" && (
            <div
          ...
          >
            <div className="tab-bar">
              {[
                ...
                <button
                  key={tab.id}
                  className={`tab-item ${bottomTab === tab.id ? "active" : ""}`}
                      ...
                      return (
                        ...
                        >
                          ...
                          <td style={{ padding: "7px 12px" }}>
                            <span
                              className="font-mono-data"
                              style={{ fontSize: 10, color: "#A855F7" }}
                            >
                          ...
                          <td style={{ padding: "7px 12px" }}>
                            ...
                            >
                              {Math.max(
                                0,
                                Math.ceil((trade.expiryAt - Date.now()) / 1000),
                              )}
                              s
                            </span>
                          </td>
                          <td style={{ padding: "7px 12px" }}>
                            ...
                            >
                              {trade.payoutMultiplier.toFixed(2)}x
                            </span>
                          </td>
                          <td style={{ padding: "7px 12px" }}>
                            <button
                              onClick={() => forceCloseTrade(trade.id)}
                              style={{
                                fontSize: 10,
                                fontWeight: 700,
                                padding: "3px 8px",
                            ...
                            style={{
                              ...
                              background:
                                trade.result === "won"
                                  ...
                                  : "rgba(239,68,68,0.12)",
                              color:
                                trade.result === "won" ? "#22C55E" : "#EF4444",
                            }}
                          >
        ...
        >
          <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
            <button
              onClick={() => {
                setContractType("rise-fall")
                setPrediction("rise")
              }}
              className={`order-side-btn order-buy ${
                contractType === "rise-fall" ? "active" : ""
              }`}
            >
              RISE/FALL
            </button>
            <button
              onClick={() => {
                setContractType("even-odd")
                setPrediction("even")
              }}
              className={`order-side-btn order-sell ${
                contractType === "even-odd" ? "active" : ""
              }`}
            >
              EVEN/ODD
            </button>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <select
                ...
                onChange={(e) => {
                  const value = e.target
                    .value as "rise-fall" | "even-odd" | "match-differ"
            ...
            <div>
              ...
              <select
                ...
                value={prediction}
                onChange={(e) =>
                  setPrediction(e.target.value as typeof prediction)
                }
              >
          ...
          >
            {[
              {
                label: "Stake",
                val: `$${parseFloat(stake || "0").toFixed(2)}`,
              },

              {
                label: "Potential Payout",
                val: `~$${(parseFloat(stake || "0") * payoutMultiplier).toFixed(2)}`,
              },
              ...
              >
                ...
                <span
                  className="font-mono-data"
                  style={{ fontSize: 12, color: "#A1A1AA", fontWeight: 600 }}
                >
                  {row.val}
                </span>
              </div>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForcedOutcome(
                        e.target.value as "none" | "won" | "lost",
                      )
                    }
                  >
                    <option value="none">No override</option>
                ...
                >
                  ...
                  Last applied:{" "}
                  {adminPolicy.lastAppliedAt
                    ? new Date(adminPolicy.lastAppliedAt).toLocaleTimeString()
                    : "Never"}
                </div>
              </div>
            </div>
