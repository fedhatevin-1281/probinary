...
export default function Wallet() {
  ...
  const [typeFilter, setTypeFilter] =
    useState<"all" | WalletTransaction["type"]>("all")
  ...
  const filteredTransactions = useMemo(() => {
    ...
    rows = [...rows].sort((a, b) => {
      ...
      return sortDir === "desc"
        ? b.balanceAfterUsd - a.balanceAfterUsd
        : a.balanceAfterUsd - b.balanceAfterUsd
    })
  ...
  return (
    ...
    >
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 13, color: "#71717A", margin: "6px 0 0" }}>
            Primary currency: USD · Mobile Money API conversion: 1 USD ={" "}
            {walletSettings.usdKesRate.toFixed(2)} KES
          </p>
        </div>
      ...
      >
        <div className="card-base" style={{ padding: 16 }}>
          <div style={{ fontSize: 12, color: "#A78BFA" }}>
            Real Wallet Balance (USD)
          </div>
          <div
            className="font-mono-data"
            style={{ marginTop: 8, color: "#FFFFFF", fontSize: 22 }}
          >
            {formatUsdValue(animatedReal)}
          </div>
        </div>
        <div className="card-base" style={{ padding: 16 }}>
          <div style={{ fontSize: 12, color: "#A78BFA" }}>
            Demo Wallet Balance (USD)
          </div>
          <div
            className="font-mono-data"
            style={{ marginTop: 8, color: "#FFFFFF", fontSize: 22 }}
          >
            {formatUsdValue(animatedDemo)}
          </div>
        </div>
        <div className="card-base" style={{ padding: 16 }}>
          <div style={{ fontSize: 12, color: "#A78BFA" }}>
          ...
          <div
            className="font-mono-data"
            style={{ marginTop: 8, color: "#22C55E", fontSize: 22 }}
          >
            {formatUsdValue(animatedAvailable)}
          </div>
        </div>
        <div className="card-base" style={{ padding: 16 }}>
          <div style={{ fontSize: 12, color: "#A78BFA" }}>
          ...
          <div
            className="font-mono-data"
            style={{ marginTop: 8, color: "#F59E0B", fontSize: 22 }}
          >
            {formatUsdValue(animatedPending)}
          </div>
        </div>
      </div>
      ...
      >
        <h3
          className="font-display"
          style={{ margin: 0, fontSize: 16, color: "#EDE9FE" }}
        >
          Mobile Money Exchange Configuration
        </h3>
        <div
          style={{
            display: "flex",
        ...
        >
          ...
          <button
            ...
            onClick={() => setUsdKesRate(Number(rateInput))}
          >
            Save Rate
          </button>
        </div>
        <div style={{ fontSize: 12, color: "#C4B5FD" }}>
          Mobile Money conversion preview: {formatUsdValue(1)} ={" "}
          {formatKesValue(walletSettings.usdKesRate)}
        </div>
      </div>
      ...
      >
        ...
        >
          <h3
            ...
            style={{ margin: 0, fontSize: 16, color: "#EDE9FE" }}
          >
            Wallet Transactions (USD)
          </h3>
          <button className="nav-feature-secondary" onClick={exportCsv}>
            Export CSV
          </button>
        </div>
        ...
        >
          <input
            className="field-input"
            placeholder="Search ID/reference/type"
            value={search}
          ...
          <select
            ...
            onChange={(event) =>
              setTypeFilter(
                event.target.value as "all" | WalletTransaction["type"],
              )
            }
          >
          ...
          <select
            ...
            value={sortBy}
            onChange={(event) =>
              setSortBy(event.target.value as "time" | "amount" | "balance")
            }
          >
              ...
              {filteredTransactions.map((item) => (
                ...
                >
                  ...
                  <td
                    style={{
                      padding: "10px 12px",
                      color: item.amountUsd >= 0 ? "#22C55E" : "#F87171",
                      fontSize: 12,
                    }}
                  >
                    {item.amountUsd >= 0 ? "+" : "-"}
                    {formatUsdValue(Math.abs(item.amountUsd))}
                  </td>
                  <td
                    style={{
                      padding: "10px 12px",
                  ...
                  >
                    {formatUsdValue(item.balanceAfterUsd)}
                  </td>
                </tr>
              ))}
