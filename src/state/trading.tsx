...
export const MARKET_SEEDS: MarketSeed[] = [
  ...
  { symbol: 'VOL-25', name: 'Volatility 25 Index', category: 'volatility', basePrice: 5234.44, volatility: 0.0021, drift: 0.00012, volatilityLabel: 'Low' },
  { symbol: 'VOL-50', name: 'Volatility 50 Index', category: 'volatility', basePrice: 6789.31, volatility: 0.0026, drift: 0.0001, volatilityLabel: 'Medium' },
  { symbol: 'VOL-75', name: 'Volatility 75 Index', category: 'volatility', basePrice: 8243.12, volatility: 0.0031, drift: 0.00008, volatilityLabel: 'High' },
  { symbol: 'VOL-100', name: 'Volatility 100 Index', category: 'volatility', basePrice: 15420.33, volatility: 0.0038, drift: 0.00008, volatilityLabel: 'Very High' },
  { symbol: 'BOOM-300', name: 'Boom 300 Index', category: 'boom', basePrice: 7841.2, volatility: 0.0028, drift: 0.0006, volatilityLabel: 'Medium' },
  { symbol: 'BOOM-500', name: 'Boom 500 Index', category: 'boom', basePrice: 12891.44, volatility: 0.0036, drift: 0.0009, volatilityLabel: 'High' },
  { symbol: 'BOOM-1000', name: 'Boom 1000 Index', category: 'boom', basePrice: 22104.55, volatility: 0.0043, drift: 0.0013, volatilityLabel: 'Very High' },
  { symbol: 'CRASH-300', name: 'Crash 300 Index', category: 'crash', basePrice: 5674.8, volatility: 0.0028, drift: -0.0005, volatilityLabel: 'Medium' },
  { symbol: 'CRASH-500', name: 'Crash 500 Index', category: 'crash', basePrice: 4312.2, volatility: 0.0034, drift: -0.0008, volatilityLabel: 'High' },
  { symbol: 'CRASH-1000', name: 'Crash 1000 Index', category: 'crash', basePrice: 2891.66, volatility: 0.0042, drift: -0.0012, volatilityLabel: 'Very High' },
  { symbol: 'STEP-UP', name: 'Step Index (Up)', category: 'step', basePrice: 9122.64, volatility: 0.0023, drift: 0.00035, volatilityLabel: 'Low' },
  { symbol: 'JUMP-10', name: 'Jump 10 Index', category: 'step', basePrice: 18934, volatility: 0.003, drift: 0.00045, volatilityLabel: 'High' },
  { symbol: 'JUMP-25', name: 'Jump 25 Index', category: 'step', basePrice: 44211.8, volatility: 0.0036, drift: 0.00065, volatilityLabel: 'Very High' },
  { symbol: 'JUMP-50', name: 'Jump 50 Index', category: 'step', basePrice: 91234, volatility: 0.0042, drift: 0.0008, volatilityLabel: 'Extreme' },
]
...
const SAMPLE_CLOSED_TRADES: BinaryTrade[] = [
  makeSampleTrade('TRD-8821', 'VOL-75', 'Volatility 75 Index', 'rise-fall', 'rise', 50, 8120.44, 8243.12, 8, 30, 1.82, 'won', 41, 0),
  makeSampleTrade('TRD-8820', 'BOOM-500', 'Boom 500 Index', 'rise-fall', 'fall', 40, 13201, 12891.44, 5, 30, 1.82, 'won', 32.8, 1),
  makeSampleTrade('TRD-8819', 'CRASH-300', 'Crash 300 Index', 'even-odd', 'odd', 35, 5890.2, 5674.8, 2, 20, 1.8, 'lost', -35, 2),
  makeSampleTrade('TRD-8818', 'VOL-100', 'Volatility 100 Index', 'match-differ', 'differ', 60, 15100, 15420.33, 0, 45, 2.15, 'won', 69, 3),
]
...
export function TradingProvider({ children }: { children: ReactNode }) {
    ...
    const connect = () => {
      ...
      socket.onmessage = event => {
        try {
          ...
          setState(prev => ({ ...prev, lastError: 'Received malformed backend message' }))
        }
      }
  ...
  const selectMarket = (symbol: string) => {
    setState(prev => (prev.selectedSymbol === symbol ? prev : { ...prev, selectedSymbol: symbol, lastAction: `Selected ${symbol}` }))
  ...
  const forceCloseTrade = (tradeId: string) => {
    ...
    if (connectionMode === 'server' && socket?.readyState === WebSocket.OPEN) {
      ...
      setState(prev => ({ ...prev, lastAction: `Requested close for ${tradeId}`, lastError: null }))
      return { ok: true }
    }
  ...
  const adjustBalance = (amount: number, reason: string) => {
    ...
    const socket = socketRef.current
    if (connectionMode === 'server' && socket?.readyState === WebSocket.OPEN) {
      socket.send(JSON.stringify({ type: 'ADJUST_BALANCE', amount: delta, reason }))
      setState(prev => ({
        ...prev,
  ...
  const equity = round2(state.balance + state.openTrades.reduce((sum, trade) => sum + trade.stake * 0.35, 0))
  const recentTrades = [...state.closedTrades].sort((a, b) => (b.settledAt ?? b.createdAt) - (a.settledAt ?? a.createdAt)).slice(0, 8)
...
export function buildSeriesShape(values: number[], width: number, height: number, padding = 10) {
...
function createInitialState(): TradingState {
  const markets = Object.fromEntries(MARKET_SEEDS.map(seed => [seed.symbol, createMarket(seed)])) as Record<string, MarketSnapshot>
...
function advanceState(prev: TradingState, now: number): TradingState {
  ...
  for (const seed of MARKET_SEEDS) {
    ...
    const price = nextPrice(current.price, seed.volatility, seed.drift, seed.basePrice)
...
function settleTrade(prev: TradingState, tradeId: string, exitPrice: number, settledAt: number): TradingState {
...
function resolveTrade(trade: BinaryTrade, exitPrice: number, settledAt: number): BinaryTrade {
...
function isWinningTrade(trade: BinaryTrade, exitPrice: number, exitDigit: number) {
  switch (trade.contractType) {
    case 'rise-fall':
      return trade.direction === 'rise' ? exitPrice > trade.entryPrice : exitPrice < trade.entryPrice
    case 'even-odd':
      return trade.direction === 'even' ? exitDigit % 2 === 0 : exitDigit % 2 === 1
    case 'match-differ':
      return trade.direction === 'match' ? exitDigit === trade.entryDigit : exitDigit !== trade.entryDigit
  }
}

function nextPrice(price: number, volatility: number, drift: number, mean: number, step = 0) {
  ...
  const delta = price * (drift + meanReversion + marketPulse + shock + spikeChance)
