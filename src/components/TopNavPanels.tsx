      ...
      >
        ...
        <tbody>
          {rows.map((row) => (
            ...
            >
              <td
                style={{ padding: "10px 12px", color: "#DDD6FE", fontSize: 12 }}
              >
                {formatDate(row.date)}
              </td>
              <td
              ...
              <td
                style={{
                  padding: "10px 12px",
                  color: row.amount >= 0 ? "#22C55E" : "#F87171",
                  fontSize: 12,
                }}
              >
                {row.amount >= 0 ? "+" : "-"}
                {formatUsdValue(Math.abs(row.amount))}
              </td>
              <td style={{ padding: "10px 12px" }}>
                <span
                  style={{
                    ...
                    border: `1px solid ${getStatusColor(row.status)}55`,
                  }}
                >
              ...
              <td
                style={{ padding: "10px 12px", color: "#A78BFA", fontSize: 12 }}
              >
                {row.market}
              </td>
              <td
                style={{ padding: "10px 12px", color: "#C4B5FD", fontSize: 12 }}
              >
                {row.referenceNumber}
              </td>
              <td
                style={{ padding: "10px 12px", color: "#DDD6FE", fontSize: 12 }}
              >
                {formatUsdValue(row.balanceAfter)}
              </td>
            </tr>
          ))}
        </tbody>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button className="nav-feature-close" onClick={onClose}>
          Close
        </button>
      </div>
      ...
      >
        <div style={sectionCardStyle}>
          ...
          <div className="font-mono-data" style={{ marginTop: 6 }}>
            {formatUsdValue(balances.real)}
          </div>
        </div>
        <div style={sectionCardStyle}>
          <div style={{ fontSize: 11, color: "#C4B5FD" }}>
          ...
          <div className="font-mono-data" style={{ marginTop: 6 }}>
            {formatUsdValue(balances.available)}
          </div>
        </div>
        <div style={sectionCardStyle}>
          <div style={{ fontSize: 11, color: "#C4B5FD" }}>Pending Balance</div>
          <div className="font-mono-data" style={{ marginTop: 6 }}>
            {formatUsdValue(balances.pending)}
          </div>
        </div>
        <div style={sectionCardStyle}>
          <div style={{ fontSize: 11, color: "#C4B5FD" }}>
          ...
          <div className="font-mono-data" style={{ marginTop: 6 }}>
            {formatUsdValue(withdrawMinimumUsd)}
          </div>
        </div>
      </div>
      ...
      >
        <div className="nav-feature-form-grid">
          ...
          <button
            className="nav-feature-primary"
            onClick={() => void submit()}
            disabled={loadingStates.withdraw}
          >
            {loadingStates.withdraw ? "Submitting..." : "Submit Withdrawal"}
          </button>
        </div>

        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <h3 style={{ margin: 0, color: "#DDD6FE", fontSize: 14 }}>
            Withdrawal History
          </h3>
          <div
            style={{
              ...
              >
                ...
                >
                  <strong
                    ...
                    style={{ fontSize: 12, color: "#DDD6FE" }}
                  >
                    {item.referenceNumber}
                  </strong>
                  <span
                    style={{ fontSize: 11, color: getStatusColor(item.status) }}
                  >
      ...
      >
        <div className="nav-feature-form-grid">
          ...
          <button
            className="nav-feature-primary"
            onClick={() => void submit()}
            disabled={loadingStates.deposit || belowMinimum}
          >
            {loadingStates.deposit ? "Processing..." : "Complete Deposit"}
          </button>
        </div>
          ...
          >
            {depositHistory.map((item) => (
              ...
              >
                ...
                <div style={{ fontSize: 12, color: "#C4B5FD", marginTop: 3 }}>
                  {item.method} · {formatUsdValue(item.amountUsd)}
                </div>
                <div style={{ fontSize: 12, color: "#A78BFA", marginTop: 3 }}>
                  {formatDate(item.createdAt)} · Ref {item.referenceNumber}
                </div>
              </div>
            ))}
          </div>
...
function HistoryPanel({ onClose }: { onClose: () => void }) {
  ...
  useEffect(
    () => setPage(1),
    [tab, query, status, fromDate, toDate, sortKey, sortDirection],
  )
      ...
      >
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {([
            ...
            <button
              key={value}
              className={`nav-chip-btn ${tab === value ? "active" : ""}`}
        ...
        >
          <input
            className="field-input"
            placeholder="Search by type, market, reference"
            value={query}
        ...
        >
          <button
            className="nav-feature-secondary"
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <strong style={{ color: "#EDE9FE", fontSize: 13 }}>
            AI Assistant
          </strong>
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button className="nav-feature-icon" onClick={onClose}>
              ×
            </button>
          </div>
        </div>
          ...
          <>
            ...
            >
              <input
                ...
                placeholder="Ask about indicators, markets, education..."
              />
              <button
                className="nav-feature-primary"
                style={{ padding: "8px 12px", borderRadius: 10, minWidth: 80 }}
                onClick={() => {
                  const text = prompt.trim()
                  if (!text) return
                  setPrompt("")
                  void sendAiPrompt(text)
                }}
              >
                Send
              </button>
  ...
  const filteredMessages = useMemo(() => {
    ...
    return chatMessages.filter(
      (message) =>
        ...
        (message.attachmentName || "").toLowerCase().includes(q),
    )
  }, [chatMessages, search])
  ...
  return (
    <aside
      style={{
        ...
        background: "#120E24",
        borderLeft: "1px solid rgba(167,139,250,0.26)",
        zIndex: 115,
      }}
    >
      <div
      ...
      >
        <div>
          ...
          <div
            style={{
              fontSize: 12,
              color: chatAgentOnline ? "#22C55E" : "#F59E0B",
            }}
          >
            {chatAgentOnline ? "Agent online" : "Leave us a message"}
          </div>
        </div>
        <button className="nav-feature-close" onClick={onClose}>
          Close
      ...
      >
        {filteredMessages.map((message) => (
          <div
            key={message.id}
            style={{
              alignSelf: message.by === "user" ? "flex-end" : "flex-start",
              maxWidth: "85%",
              background:
                message.by === "user"
          ...
          >
            {message.text}
            <div style={{ marginTop: 4, fontSize: 10, color: "#C4B5FD" }}>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <input
            ...
            placeholder={
              chatAgentOnline ? "Type your message" : "Leave us a message"
            }
          />
          <button
            className="nav-feature-primary"
            ...
            onClick={() => {
              ...
              if (!msg) return
              setText("")
              void sendSupportMessage(msg)
            }}
            disabled={loadingStates.chatSend}
          >
            {loadingStates.chatSend ? "Sending..." : "Send"}
          </button>
        </div>
      </div>
    </aside>
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: 16, display: "grid", gap: 10 }}>
        ...
        />
        <div
          style={{ maxHeight: 430, overflowY: "auto", display: "grid", gap: 8 }}
        >
          {filtered.map((item) => (
            <div
              key={item.id}
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <strong style={{ color: "#EDE9FE", fontSize: 14 }}>
                  {item.title}
                </strong>
                <button
                  className="nav-feature-icon"
                  onClick={() => toggleLessonBookmark(item.id)}
                >
                  {item.bookmarked ? "★" : "☆"}
                </button>
              </div>
              <div style={{ fontSize: 12, color: "#C4B5FD" }}>
                {item.section} · {item.duration} · {item.difficulty}
              </div>
              {!item.completed && (
                <button
                  className="nav-feature-secondary"
...
function NotificationsPanel({ onClose }: { onClose: () => void }) {
  const { notifications, markAllNotificationsRead, deleteNotification } =
    useTopNav()
    ...
    >
      ...
      <div style={{ padding: 10, display: "flex", flexWrap: "wrap", gap: 6 }}>
        {([
          ...
          <button
            key={value}
            className={`nav-chip-btn ${category === value ? "active" : ""}`}
      ...
      <div style={{ height: "calc(100% - 132px)", overflowY: "auto" }}>
        {visible.map((item) => (
          ...
          >
            ...
            >
              <strong style={{ fontSize: 12, color: "#EDE9FE" }}>
                {item.title}
              </strong>
              <button
                className="nav-feature-icon"
